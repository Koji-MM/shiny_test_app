...
# Input data with Slidebarlayout 
    sidebarLayout(
      sidebarPanel(
        ...
        br(""),
        selectInput("Choi1",h4("特性を表示する形質"),choices=list("形質1"=6,"形質2"=7,"形質3"=8, "形質4"=9,"形質5"=10),selected = 1),
 ...
 # Main panel
      mainPanel(
        tabsetPanel(type="tabs",
                    ...
                    tabPanel("クローン血縁Map",h4("0.00:血縁関係なし, 0.50:全兄弟or親子, 0.25:半兄弟"),plotOutput("ICM2")),
                    )))),
  tabPanel("シミュレーション",
           h2("シミュレーションの設定と実行"),
           sidebarLayout(
...
tabPanel("使い方",
         h4("簡易な使い方"),
         br(""),
         h4("1:「植栽クローン」タブに移動し、クローン特性表を取り込む"),
         h4("2:「シミュレーション」タブに移動し、採種園のサイズと植栽クローンと本数を入力する"),
         h4("　　シミュレーション回数を選択し、「実行」する"),
         h4("　　Result.csvファイルが自動的に出力される"),
         h4("3:「シミュレーション結果」タブに移動し、Result.csvファイルを読み込む"),
         h4("　　配置タブに、近親交配を抑制した配置が出力される"),
         h4("4:「配置をダウンロード」ボタンで配置をダウンロードする"),
         br(""),
         br(""),
         h4("クローン特性表は一レコード（系統）、一行のフォーマットで整え、CSV形式で保存しておく"),
         h4("列は左端から、系統名、通し番号、系統コード、母親系統コード、父親系統コード、形質1～5の順"),
         h4("現在のバージョンでは血縁関係のみに依存して配置を決定します。形質情報はなくても構いません")
         )
)
